...
ZTEST(framework_tests, test_arm_scenario) {
  ...
  receive_command(fd, NET_CMD_DEVICE_REGISTERED);
  receive_command(fd, NET_CMD_DEVICE_REGISTERED);
  ...
  LOG_INF("Setting button GPIO input to 1");
  ftest_gpio_emul_input_set(BUTTON_GPIO, FTEST_BUTTON_PIN, 1);
